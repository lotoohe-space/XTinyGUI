...
uint8 GUICursorDrawMem(int16 x, int16 y, uintColor color) {
	if (x >= GUICursor.cursorPOI.x && x < GUICursor.cursorPOI.x + GUICursor.GUICursorPTBitmap.w
		&& y >= GUICursor.cursorPOI.y && y < GUICursor.cursorPOI.y + GUICursor.GUICursorPTBitmap.h
		) {
		int16 drawX, drawY;
