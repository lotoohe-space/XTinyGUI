...
PUBLIC HWIN_HEAD WINDOWS_HEAD_MARK_HEAD(Create)(char *title, int16 x, int16 y, int16 w, int16 h)
{
	...
	/*添加头标题*/
	WINDOWS_HEAD_MARK_HEAD(Add)
	(hWinHead, hTextWidge);
	/*添加最小化BUTTON*/
	WINDOWS_HEAD_MARK_HEAD(Add)
	(hWinHead, hWinHead->hXButtonMin);
	/*添加最大化BUTTON*/
	WINDOWS_HEAD_MARK_HEAD(Add)
	(hWinHead, hWinHead->hXButtonMax);
	WINDOWS_HEAD_MARK_HEAD(Add)
	(hWinHead, hWinHead->hXButtonClose);
...
PRIVATE void WINDOWS_HEAD_MARK_HEAD(Paint)(void *hObject)
{
	...
	if (!DrawSetArea((HWIDGET_BASE)hWinHead))
	{
		return;
	} // 计算得到当前绘图区域
	// 计算得到剪裁区域
